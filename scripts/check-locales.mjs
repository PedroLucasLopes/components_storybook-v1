...
for (const file of files) {
  ...
  if (file !== 'en.json') {
    for (const key of reference.keys()) {
      if (!messages.has(key)) problems.push(`${file}: falta "${key}"`);
    }
  }
}
...
if (sources) {
  ...
  for (const file of code) {
    ...
    for (const match of text.matchAll(/(['`])([a-z][A-Za-z0-9]*(?:\.[A-Za-z0-9_]+)+)\1/g)) {
      ...
      else problems.push(`${relative(process.cwd(), file)}: "${key}" não existe no en.json`);
    }

    for (const match of text.matchAll(/`([a-z][A-Za-z0-9]*(?:\.[A-Za-z0-9_]+)*\.)\$\{/g)) {
